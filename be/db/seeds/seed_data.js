...
exports.seed = async function (knex) {
    await knex('trending_images').del();
    await knex('regular_images').del();
  ...
  const movieData = [
    ...
    for (const movie of movieData) {
      const [mediaId] = await knex('movie_data').insert({
        title: movie.title,
        year: movie.year,
      ...
      if (movie.thumbnail.trending) {
        await knex('trending_images').insert({
          media_id: mediaId,
      ...
      if (movie.thumbnail.regular) {
        await knex('regular_images').insert({
          media_id: mediaId,
